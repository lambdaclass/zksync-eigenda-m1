...
pub fn prove_risc0_proof(session_info: ProveInfo, private_key: String, blob_index: u32, chain_id: String) -> anyhow::Result<()>{
    ...
    if output.status.success() {
        ...
        let transactions = json.get("transactions").and_then(|t| t.as_array()).ok_or(anyhow::anyhow!("Invalid JSON structure: 'transactions' not found or not an array"))?;
        let first_transaction = transactions.first().ok_or(anyhow::anyhow!("Invalid JSON structure: 'transactions' array is empty"))?;
        let tx_hash = first_transaction.get("hash").and_then(|h| h.as_str()).ok_or(anyhow::anyhow!("Invalid JSON structure: 'hash' not found or not a string"))?;
        println!("Proof of data inclusion for blob {} verified on L1. Tx hash: {tx_hash}", blob_index);
        return Ok(())
    } else {
        println!("Proof verification failed: {:?}", std::str::from_utf8(&output.stderr).unwrap());
